...
- ``dro_metric: str = "wasserstein"`` -- the only accepted v0.7.2 value. It
...
- ``leaf_model: str = "constant"``
  ...
  - ``"linear"`` -- each leaf stores a small linear model
    ...
    user-tunable in v0.7.2). Optimal weights are solved in closed form via the

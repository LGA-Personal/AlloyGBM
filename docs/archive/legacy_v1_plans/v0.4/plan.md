...
## Summary
- Goal: deliver the `0.4.0` milestone by improving CPU training-kernel performance and introducing SIMD acceleration readiness on top of verified `v0.3` evaluation/validation tooling.
- Success criteria:
...
- Audience: engineers implementing `v0.4` child slices and reviewers gating progression toward `v0.5` model IO and predictor integration.
...
- Model-format/predictor compatibility policy changes (`v0.5+` scope).
...
- no breaking changes to `GBMRegressor` public API or existing metric/split helper signatures from `v0.3`.
...
1. Execute `docs/architecture/v1.0/v0.4/v0.4.1/plan.md` to establish benchmark baseline and first low-risk kernel optimization slice.
...
## Acceptance Criteria
1. `v0.4` child slices collectively deliver benchmark harness coverage for backend CPU histogram/split hot paths with reproducible fixture definitions.
...
5. Existing engine/predictor/wrapper regression tests remain green throughout `v0.4` execution.
...
- Risk: scope creep into non-`v0.4` milestones.

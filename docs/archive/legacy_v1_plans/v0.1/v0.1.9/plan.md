# AlloyGBM v0.1.9 Plan (v0.1 Python Runtime Success-Path Parity)

## Objective
Close the remaining `v0.1` runtime-evidence gap by adding Python-runtime tests that execute predictor inference on valid artifact bytes and verify parity between direct native entrypoint execution and the public `GBMRegressor.predict_from_artifact(...)` bridge.
...
- Out of scope:
  ...
  - Parent-layer rollup artifacts for `v0.1` and `v1.0`.
...
2. Verification package:
  - `docs/architecture/v1.0/v0.1/v0.1.9/implementation_notes.md`
  - `docs/architecture/v1.0/v0.1/v0.1.9/verification_report.md`
3. State package:

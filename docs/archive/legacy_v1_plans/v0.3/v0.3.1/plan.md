# AlloyGBM v0.3.1 Technical Plan (v0.3 Baseline Metrics Slice)

## Summary
- Goal: execute the first `v0.3` child slice by adding a decision-complete baseline metric API (`RMSE`, `MAE`, `R2`, Pearson correlation) for wrapper workflows.
- Success criteria:
  ...
  - all existing `v0.2` wrapper/runtime gates remain green.
- Audience: engineers implementing `v0.3.1` and reviewers validating readiness for later `v0.3` slices (finance metrics and leakage splits).
...
- Produce layer artifacts:
  - `docs/architecture/v1.0/v0.3/v0.3.1/implementation_notes.md`
  - `docs/architecture/v1.0/v0.3/v0.3.1/verification_report.md`

### Out of Scope
- Purged K-fold / embargo split helpers (later `v0.2.x` slices).
...
4. Existing `v0.2` regressor contract/runtime tests continue to pass unchanged.

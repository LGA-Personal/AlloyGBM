...
## Layer
- Path: `docs/architecture/v1.0/v0.3/v0.3.1`
- Date: 2026-02-27
...
- Criterion: (4) Existing `v0.2` regressor contract/runtime tests remain passing unchanged.
- Evidence:
...
- Finance-specific ranking metrics and leakage split helpers are deferred to later `v0.3` child slices.

## Final Readiness
- Ready: Yes (`v0.3.1` acceptance criteria satisfied).
- Required follow-up before merge/release: update `docs/architecture/state/layer_index.yaml` to mark `docs/architecture/v1.0/v0.3/v0.3.1` as `verified`.

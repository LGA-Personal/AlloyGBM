# AlloyGBM v0.8.1 Plan (v0.8 Hardening Matrix Baseline Slice)

## Summary
- Goal: execute `v0.8.1` as the first `v0.8` child slice by locking a decision-complete release hardening matrix, baseline evidence inventory, and verification protocol for subsequent `v0.7.x` slices.
- Success criteria:
  - a concrete hardening matrix artifact exists and maps release gates to evidence sources,
  - baseline compatibility and behavior inventory from `v0.7` closeout is captured with traceable links,
  - `v0.8.1` artifacts are complete and the next child target advances to `v0.8.2`.
- Audience: engineers implementing `v0.8` hardening slices and reviewers gating `1.0.0` readiness evidence quality.

## Scope
### In Scope
- Author `v0.8.1` layer artifacts:
  - `docs/architecture/v1.0/v0.8/v0.8.1/plan.md`
  - `docs/architecture/v1.0/v0.8/v0.8.1/implementation_notes.md`
  - `docs/architecture/v1.0/v0.8/v0.8.1/verification_report.md`
...
- Hardening matrix content requirements:
  ...
  - baseline evidence sources from `v0.7` child/parent verification artifacts,
...
- State progression update:
  ...
  - create/advance next target to `docs/architecture/v1.0/v0.8/v0.8.2`.

### Out of Scope
- New algorithmic features or roadmap expansion.
- Model-format version changes.
- Python/Rust public API redesigns.
- Parent `v0.8` rollup artifact completion (`implementation_notes.md`, `verification_report.md` at parent level).
...
- Existing code interfaces remain unchanged:
  - SHAP APIs and behavior baseline from `v0.7`,
...
- all behavior commitments from `v0.7` remain the reference baseline for later hardening slices.

## Implementation Sequence
1. Build a `v0.8` hardening baseline from ancestor and `v0.7` closeout artifacts.
...
## Acceptance Criteria
1. `docs/architecture/v1.0/v0.8/v0.8.1/plan.md` is present and decision-complete for the hardening-matrix slice.
2. `docs/architecture/v1.0/v0.8/v0.8.1/release_hardening_matrix.md` exists and maps release gates to concrete evidence sources.
3. Matrix includes explicit baseline non-regression commitments carried from `v0.7`.
4. `docs/architecture/v1.0/v0.8/v0.8.1/implementation_notes.md` is present and scoped to this slice.
5. `docs/architecture/v1.0/v0.8/v0.8.1/verification_report.md` is present with criterion-to-evidence status.
...
11. `docs/architecture/state/layer_index.yaml` marks `v0.8.1` verified and advances next target to `docs/architecture/v1.0/v0.8/v0.8.2`.
...
- Next child target after this slice is `docs/architecture/v1.0/v0.8/v0.8.2`.
- Full verification command gates remain the default evidence standard for `v0.8` children.

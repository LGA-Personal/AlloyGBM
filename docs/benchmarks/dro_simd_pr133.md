...
| Baseline benchmark commit | `4c7a333afd08526309f02e87f0f7a395010af63d` |
| Candidate benchmark commit | `90146ca3c2d616bca7149c835d82467d931ca879` |
...
| --- | ---: | ---: | ---: |
| 16 bins | 22,573.50 | 22,154.75 | 1.019x |
| 64 bins | 24,767.33 | 24,055.83 | 1.030x |
| 255 bins | 185,542.75 | 151,882.17 | 1.222x |

The scanner gate requires at least 1.5x at both 64 and 255 bins, so this
microbenchmark gate did not pass. The final candidate includes only the declared
...
eight-bin block. The final implementation was retained for the end-to-end result;
the rejected prototype timings are listed below.
...
| --- | --- | --- | ---: | ---: | --- | ---: | ---: |
| `reg-small-narrow` | regression | small-narrow | 640 x 8 | 80 | RMSE | 0.814644 | 0.993904 |
| `reg-small-wide` | regression | small-wide | 640 x 128 | 80 | RMSE | 0.756167 | 1.000046 |
| `reg-tall-narrow` | regression | tall-narrow | 8192 x 16 | 80 | RMSE | 0.771367 | 0.996380 |
| `reg-tall-wide` | regression | tall-wide | 8192 x 128 | 60 | RMSE | 0.766504 | 1.000939 |
| `reg-noisy` | regression | medium | 2048 x 32 | 100 | RMSE | 0.852997 | 1.000543 |
| `binary-imbalanced` | binary | medium | 4096 x 32 | 100 | log loss | 0.846953 | 1.001239 |
| `multiclass-wide` | multiclass | small-wide | 2048 x 96 | 80 | log loss | 0.783878 | 0.999532 |
| `rank-small-query` | ranking | tall-narrow | 2400 x 24, 120 queries | 60 | NDCG@10 | 0.813883 | 0.997599 |
| `rank-large-query` | ranking | tall-narrow | 4096 x 24, 8 queries | 60 | NDCG@10 | 0.913097 | 1.012264 |
...
| --- | ---: |
| medium | 0.850071 |
| small-narrow | 0.814644 |
| small-wide | 0.769634 |
| tall-narrow | 0.817880 |
| tall-wide | 0.766504 |

The aggregate median is `0.814644`, or an `18.5356%` DRO fit-time improvement.
The median native-training stage ratio across the 45 DRO records is `0.811951`;
the corresponding total-fit ratio is `0.813189` when computed directly across
records. Input adaptation and native bridge preparation medians were `1.001441`
and `0.988553`, respectively. The comparison's declared shape-median aggregate
is the acceptance value.

The 5% shape-regression guard passed for every DRO shape. The 15% end-to-end DRO
fallback also passed. The strict standard-arm sentinel did not pass: its median
ratio was `1.000046`, but the worst individual standard record was `1.042893`,
above the allowed `1.03`. The machine-readable comparison therefore correctly
records `passed: false`, despite the DRO fallback improvement and exact quality
equivalence.
...
The baseline command must be run at the production base or the immutable baseline
artifact. The committed baseline JSON identifies its source commit; the candidate
JSON identifies `90146ca`.

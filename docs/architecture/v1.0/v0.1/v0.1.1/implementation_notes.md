...
- Implemented the first executable `v0.1` child slice at `docs/architecture/v1.0/v0.1/v0.1.1` focused on control-contract lock-in.
...
- No contradictions to `docs/architecture/v1.0/v0.1/v0.1.1/plan.md` were introduced.

## Boundary/Interface Changes vs Plan
- `core` contract surface expanded exactly for `v0.1` control fields.
...
- No parent `v0.1` rollup artifacts yet (`implementation_notes.md` / `verification_report.md`).
- Full `0.1.0` scope items (deeper histogram growth breadth and broader quality/perf work) remain for subsequent child layers.

## Follow-Up Actions
- Implement `v0.1.2` to advance from deterministic sampling baseline toward stronger per-round sampling semantics and broader `v0.1` behavior completion.
- Keep validation-stop contracts stable while extending training behavior depth in subsequent `v0.1` slices.

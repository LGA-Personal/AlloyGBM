...
- Implemented `v0.1.5` as a depth-behavior slice for `v0.1` in `crates/engine/src/lib.rs`.
...
- Reason: avoids format churn in a mid-`v0.1` slice while still enabling path-aware behavior.
...
- No contradictions to `docs/architecture/v1.0/v0.1/v0.1.5/plan.md` were introduced.
...
- `v0.1` parent rollup artifacts are still pending:
...
## Follow-Up Actions
- Define `v0.1.6` around remaining `v0.1` completion gaps (for example stronger quality/parity evidence for depth-grown behavior and any remaining predictor alignment items) while preserving seeded subsampling and validation rollback semantics.

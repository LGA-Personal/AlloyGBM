# AlloyGBM v0.1.2 Plan (v0.1 Seeded Per-Round Subsampling Semantics)

## Objective
Advance `v0.1` from deterministic prefix subsampling baseline to deterministic seeded per-round row/column subsampling semantics in the engine training loop, while preserving the `v0.1.1` validation-stop contract and reproducibility guarantees for deterministic mode.

# AlloyGBM v0.1.6 Plan (v0.1 Predictor Artifact Inference Parity)

## Objective
Close a remaining `v0.1` completion gap by implementing the `predictor` crate artifact import and row/batch inference path so predictor outputs match engine outputs from the same serialized model bytes, including depth-grown/path-conditioned stump behavior introduced in `v0.1.5`.
...
- Out of scope:
  ...
  - Parent-layer (`v0.1` / `v1.0`) rollup artifacts.
...
2. Verification package:
  ...
  - `docs/architecture/v1.0/v0.1/v0.1.6/implementation_notes.md`
  - `docs/architecture/v1.0/v0.1/v0.1.6/verification_report.md`
3. State package:

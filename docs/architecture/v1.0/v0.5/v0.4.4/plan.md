...
## Summary
- Goal: execute the optional `v0.5` closeout slice by hardening the AVX2-capable histogram route for `x86_64` benchmarking, removing cross-target portability blockers, and adding reproducible AVX2-vs-scalar comparison workflow.
- Success criteria:
...
- Audience: engineers closing `v0.5` SIMD benchmark risk and reviewers validating `x86_64` measurement readiness before parent rollup.
...
- Parent `v0.5` rollup artifact authoring (handled after this slice).
- CUDA/Metal/MLX work.
...
- Parent `v0.5` remains `planned-only` until rollup artifacts are produced.

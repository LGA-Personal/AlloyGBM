...
- Completed `v0.5` through child slices `v0.4.1` -> `v0.4.4`:
...
- `v0.5` changes were confined to backend internals, benchmark harnesses, and documentation/state artifacts.
...
- Proceed with next parent target planning (`v0.6` and beyond) while preserving the caveat in release-gate decisions until native AVX2 evidence is available.

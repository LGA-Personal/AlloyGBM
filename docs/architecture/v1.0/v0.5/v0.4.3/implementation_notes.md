...
- Reason: `v0.5` requires explicit SIMD readiness without sacrificing portability and deterministic behavior on non-AVX2 hosts/CI environments.
...
- Proceed with next child slice only if further SIMD tuning is required; otherwise prepare `v0.5` parent rollup artifacts.

...
## Summary
- Goal: execute the second `v0.5` child slice by reducing `build_histograms` small-workload overhead while preserving or improving the medium-workload gain established in `v0.4.1`, and tighten split-path benchmark coverage.
- Success criteria:
...
- Predictor/model-format/API changes (`v0.6+` scope).
- External LightGBM benchmark suite onboarding (deferred to later `v0.5` closeout work).

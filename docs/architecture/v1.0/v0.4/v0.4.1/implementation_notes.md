...
- AVX2/runtime SIMD dispatch remains deferred to later `v0.3.x` slices.
...
## Follow-Up Actions
- Open `docs/architecture/v1.0/v0.4/v0.4.2/plan.md` for deeper histogram/split-path optimization.
- Keep `best_split` and histogram benchmark tracking in `histogram_kernels` for regression monitoring across `v0.3.x`.
- Update `docs/architecture/state/layer_index.yaml` to mark `docs/architecture/v1.0/v0.4/v0.4.1` as `verified`.

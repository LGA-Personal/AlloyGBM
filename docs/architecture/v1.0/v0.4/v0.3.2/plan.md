# AlloyGBM v0.3.2 Technical Plan (v0.4 Finance Metrics Slice)

## Summary
- Goal: execute the second `v0.4` child slice by adding finance-oriented evaluation metrics (`rank-IC`, `hit-rate`, `ICIR`) on top of verified `v0.3.1` baseline metrics.
- Success criteria:
  ...
  - existing `v0.3` wrapper/runtime behavior remains unchanged and all verification gates stay green.
...
- Produce layer artifacts:
  - `docs/architecture/v1.0/v0.4/v0.3.2/implementation_notes.md`
...
4. Existing `v0.3` regressor/runtime tests continue to pass unchanged.

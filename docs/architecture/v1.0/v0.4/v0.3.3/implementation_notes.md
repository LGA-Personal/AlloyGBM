...
- Added layer plan and verification artifacts:
  - `docs/architecture/v1.0/v0.4/v0.3.3/plan.md`
...
- Reason: parent `v0.4` scope requires time-aware panel-safe evaluation tooling, and time-first splitting best enforces purge/embargo leakage boundaries.
...
- Optional `v0.3.4` polish/cleanup slice is still open if parent `v0.4` needs additional API/docs hardening.
...
## Follow-Up Actions
- Update `docs/architecture/state/layer_index.yaml` to mark `docs/architecture/v1.0/v0.4/v0.3.3` as `verified`.
- Decide whether to open `v0.3.4` (if residual scope remains) or close parent `v0.4` with rollup artifacts and verification evidence.

# AlloyGBM v0.2.1 Plan (v0.2 Wrapper Native Fit/Predict Slice)

## Objective
Execute the first child step of `v0.2` by replacing scaffold-only constant-baseline estimator behavior with native-backed `fit`/`predict` flow in `GBMRegressor`, while preserving sklearn-style parameter and validation contracts.
...
- Out of scope:
  - Full multi-library dataframe adapters (broader NumPy/pandas/Polars parity can be split into later `v0.1.x` slices).
  - New ranking/categorical/SHAP capabilities.
  - Performance optimization beyond correctness-oriented wrapper behavior.
  - Parent `v0.2` and top-level `v1.0` rollup closure artifacts.
...
3. Layer documentation package:
  - `docs/architecture/v1.0/v0.2/v0.2.1/implementation_notes.md`
  - `docs/architecture/v1.0/v0.2/v0.2.1/verification_report.md`
4. State package:

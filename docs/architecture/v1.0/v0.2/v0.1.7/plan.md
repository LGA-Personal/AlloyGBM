# AlloyGBM v0.1.7 Plan (v0.2 Python Binding Predictor Bridge)

## Objective
Close a remaining `v0.2` integration gap by exposing predictor artifact inference through the Python native binding entry points and adding parity-focused evidence that Python-facing calls produce the same predictions as engine inference from the same serialized model bytes.
...
- Out of scope:
  ...
  - Parent `v0.2` and `v1.0` rollup artifacts.

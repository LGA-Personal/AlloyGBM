# AlloyGBM v0.2.3 Plan (v0.2 Estimator Round-Control Slice)

## Objective
Execute the next `v0.2` child slice by introducing explicit estimator round-count control (`n_estimators`) for `GBMRegressor`, wiring it through the Python wrapper and native training bridge while preserving existing sklearn-style contracts and validation semantics.
...
- Out of scope:
  ...
  - Parent rollup closure artifacts at `v0.2` or `v1.0`.
...
4. Layer documentation package:
  - `docs/architecture/v1.0/v0.2/v0.2.3/implementation_notes.md`
  - `docs/architecture/v1.0/v0.2/v0.2.3/verification_report.md`
5. State package:

...
- Implemented `v0.1.2` as the second `v0.2` child slice focused on replacing prefix-only subsampling semantics.
...
- No contradictions to `docs/architecture/v1.0/v0.2/v0.1.2/plan.md` were introduced.
...
- Still stump-level iterative training (full-depth behavior remains future `v0.2` slices).
- Parent rollup artifacts for `v0.2` are not complete yet.
...
## Follow-Up Actions
- Start `v0.1.3` to continue `v0.2` behavior completion (tree-growth/quality criteria beyond subsampling semantics).

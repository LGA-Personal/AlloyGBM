# AlloyGBM v0.9 Migration and Compatibility Narrative (`v0.8.4`)

## Purpose
- Finalize migration-facing guidance for `v0.9` hardening closeout.
- Convert compatibility expectations into executable checks tied to release evidence.
- Provide a checklist that can be reused during parent `v0.9` and milestone `1.0.0` go/no-go review.

## Migration Impact Summary
- Model artifact format:
  - No format version bump is introduced in `v0.9` (`v1` contract remains in effect).
...
- Runtime behavior:
  - `v0.8` SHAP behavior and predictor parity commitments remain non-regression baselines.
...
5. Confirm benchmark reproducibility evidence exists from `v0.8.3`:
   - `docs/architecture/v1.0/v0.9/v0.8.3/benchmark_run_summary.md`
...
| SHAP + estimator non-regression | SHAP/predict contract remains stable | `v0.8` child/parent verification reports | full gate reruns in `v0.8.4` |
...
- Benchmark thresholds are not enforced in CI yet; reproducibility evidence exists, but pass/fail policy remains a parent `v0.9`/later decision.
...
- Layer index advances to parent `docs/architecture/v1.0/v0.9` for rollup closeout.

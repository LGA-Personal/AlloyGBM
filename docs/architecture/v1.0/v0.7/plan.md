...
## Summary
- Goal: deliver the `0.7.0` milestone by replacing SHAP placeholders with exact TreeSHAP for CPU regression artifacts and exposing global/per-row explanation APIs in Rust and Python.
- Success criteria:
...
- Audience: engineers implementing `v0.7` child slices and reviewers gating readiness for `v0.8` hardening.
...
- Child-layer planning and execution decomposition under `docs/architecture/v1.0/v0.7/v0.6.x`.
...
7. `v0.7` child slices and parent rollup artifacts are present and linked in verification.
...
- Device scope remains CPU-only for `v0.7`.
...
- Child-layer numbering under `v0.7` uses `v0.6.x`; immediate next child target is `docs/architecture/v1.0/v0.7/v0.7.1`.

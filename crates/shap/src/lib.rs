use alloygbm_core::{LeafValue, LinearLeaf, ModelMetadata};
...
use binning::{
    ADDITIVITY_ATOL, ADDITIVITY_RTOL, MAX_EXACT_SPLIT_FEATURES, TREE_NODE_STRIDE,
    additivity_tolerance,
};
pub use error::{ShapError, ShapResult};
use types::{ModelStructure, build_model_structure, load_artifact_context};
...
///
/// Linear-leaf (PL) models are rejected by this entry point in v0.12.1 —
...
fn explain_interactions_from_model(
    ...
    if model_has_linear_leaves(model) {
        return Err(ShapError::InvalidInput(
            "SHAP interaction values are not supported for leaf_model=\"linear\" artifacts in v0.12.1"
                .to_string(),
        ));
    }

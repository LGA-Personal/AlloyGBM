...
- **Joint trainer core feature parity** (v0.10.2): the joint multi-output trainer (`crates/engine/src/joint.rs`) gained five features matching the single-output path, plus a partially-shipped sixth:
  ...
  Still deferred to v0.10.3 (native-cat Python wiring, joint GOSS / DART / warm-start) and v0.10.4 (joint MorphBoost / DRO / neutralization). The `_JOINT_SUPPORTED_KWARGS` allow-list in `multi_label_ranker.py` is the source of truth for what's permitted in joint mode at any given release.

...
AlloyGBM/
  ...
  docs/
    limitations.md         # Current limitation analysis (v0.10.1, with v0.10.x follow-ups)
...
- **Multiclass softmax + DART** (v0.10.1): `GBMClassifier(boosting_mode="dart")` works for K ≥ 3 classes via per-class `dart_round_start_offsets[k]` + `dart_round_counts[k]` arrays in `fit_multiclass_iterations_impl` (mirroring the single-output path's flat `round_start_offsets` / `dart_round_counts`). Dropout flat index `flat_idx = r * K + class_k` resolves to `class_stumps[class_k][start..start+count]` — the WHOLE class tree slice, not just one stump. Each round's `dart_round_finalize: Option<(new_w, new_dropped_weights)>` defers `dart_state` mutation + per-stump `tree_weight` stamping to the round-accept branch. Validation predictions get the full DART transition (subtract dropped → scale new at new_w → re-add dropped at w_new) for early-stopping correctness. Requires `tree_growth="level"`. Warm-start via `MultiClassWarmStartState.initial_dart_tree_weights` (flat round-major × class-k): the PyO3 bridge reconstructs per-tree weights by grouping `class_stumps[k]` by `tree_id = node_id / TREE_NODE_STRIDE` and taking the first stump's `tree_weight` per tree (mirrors `apply_dart_tree_weights` in `crates/predictor/src/lib.rs`).

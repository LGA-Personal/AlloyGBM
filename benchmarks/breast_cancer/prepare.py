#!/usr/bin/env python3
"""Prepare Breast Cancer classification benchmark data via scikit-learn."""
...
def main(argv: list[str]) -> int:
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path(__file__).resolve().parents[1] / "data" / "breast_cancer",
    ...
    with prepared_path.open("w", encoding="utf-8", newline="") as prepared_file:
        ...
        for features, target in zip(bunch.data, bunch.target):
            row = {name: value for name, value in zip(feature_names, features)}
            row["target"] = int(target)
            writer.writerow(row)

    print(f"[breast_cancer] prepared dataset written to {prepared_path}")
    return 0

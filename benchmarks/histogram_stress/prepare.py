#!/usr/bin/env python3
"""Generate synthetic benchmark data that stresses histogram kernels."""
...
def _generate_feature(row_index: int, feature_index: int, rng: random.Random) -> float:
    # Blend quantized and skewed signals to force hot bins and long tails.
    if feature_index == 0:
...
def _generate_target(features: list[float], rng: random.Random) -> float:
    ...
    return weighted + nonlinear + noise
...
def main(argv: list[str]) -> int:
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path(__file__).resolve().parents[1] / "data" / "histogram_stress",
    ...
    print(
        "[histogram_stress] prepared dataset written to "

...
from benchmarks.competitiveness.summarize import aggregate_records, render_markdown
...
def test_aggregate_uses_median_unscaled_mad_and_raw_provenance() -> None:
    ...
    assert "raw.jsonl#L1" in markdown and "raw.jsonl#L5" in markdown
...
def test_speed_gate_has_exact_ten_percent_and_one_percent_boundaries(candidate_fit: float, reference_fit: float, candidate_metric: float, status: str) -> None:
    ...
    baseline = aggregate_records(records("run-base", "baseline", fit=reference_fit, metric=1.0))[0]
...
def test_quality_requires_two_noise_clearing_scenarios_and_allows_quality_first_fit_cost() -> None:
    ...
    baseline = [
        aggregate_records(records("run-base", "reference", fit=1.0, metric=1.0, scenario=scenario))[0]
        for scenario in ("a", "b")
    ]
...
def test_catastrophic_regression_uses_strict_boundaries(metric: float, fit: float, status: str) -> None:
    ...
    baseline = aggregate_records(records("run-base", "reference", fit=1.0, metric=1.0))[0]
...
def test_deep_scaling_manifest_has_exact_27_scenarios() -> None:
    ...
    assert {item["rows"] for item in manifest["scenarios"]} == {100000, 500000, 1000000}
    assert {item["features"] for item in manifest["scenarios"]} == {20, 100, 500}
    assert {item["depth"] for item in manifest["scenarios"]} == {4, 8, 12}
